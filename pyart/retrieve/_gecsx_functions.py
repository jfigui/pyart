...
def clip_grid(grid, xr, yr, extra_m = 5000):
    ...
    for f in grid.fields.keys():
        grid.fields[f]['data'] = grid.fields[f]['data'][np.ix_(mask_y, mask_x)]
    grid.nx = len(grid.x['data'])
    grid.ny = len(grid.x['data'])
    return grid
...
def rcs(azmap, elmap, rmap, areaeffmap, sigma0map, vismap, elevations,
         DEM_res,  DEM_xmin,  DEM_ymin,  rad_x,  rad_y, rpol, azpol,  beamwidth,
         pulsewidth,  range_weighting = True,  az_conv = 0, 
         rcs_raster_oversampling = None, verbose = True):
    ...
    if rcs_raster_oversampling == None:
    ...
    elevations_rad = np.array(elevations) * np.pi / 180.
    ...
    rcspolarmap = np.zeros((nazim * len(elevations), nrange)) + np.nan
...
def sigma0(inc_ang, frequency, method = 'gabella'):
    ...
    if method == 'gabella':
        ...
        if len(ind):
            iang = inc_angr[ind]
            sigma_db[ind] = (a1 + b1 * iang + c1 * frequency_ghz +
                             d1 * frequency * iang)
        ind =inc_ang >= lim_ang_del
        
        if len(ind):
           iang = inc_angr[ind]
           sigma_db[ind] = (a1_2 + b1_2 * iang + c1_2 * frequency_ghz +
                             d1_2 * frequency * iang) 
...
def visibility(azmap, rmap, elmap, DEM_res, DEM_xmin, DEM_ymin, dr, daz, 
               rad_x, rad_y, verbose = True):
...
def visibility_angle(minviselmap, azmap, rmap, 
                     elevations, DEM_res,  DEM_xmin,  DEM_ymin,  rad_x,  
                     rad_y, rpol, azpol,  beamwidth, pulsewidth, az_conv = 0,
                     raster_oversampling = None, verbose = True):
    ...
    # pyart storage format: 2D arrays (naz * nel, nranges)
    vispol = np.zeros((nazim * len(elevations), nrange))
